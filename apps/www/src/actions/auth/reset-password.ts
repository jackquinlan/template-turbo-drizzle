...
import { forgotPasswordSchema, updatePasswordSchema } from "@repo/auth/validators";
...
export async function sendResetTokenEmailAction(values: z.infer<typeof forgotPasswordSchema>) {
  ...
  if (!existingUser || !existingUser.email) {
    return { message: "If an account with that email exists, we will send you an email with instructions to reset your password." };
  }
...
export async function resetPasswordAction(values: z.infer<typeof updatePasswordSchema>) {
