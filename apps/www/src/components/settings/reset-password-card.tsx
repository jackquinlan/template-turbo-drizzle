  ...
  async function handleSubmit(data: z.infer<typeof forgotPasswordSchema>) {
    ...
    startTransition(async () => {
      sendResetTokenEmailAction(data)
        .then((res) => {
          if (res?.message) setMessage("An email has been sent to reset your password.");
        })
        .catch((error) => {
      ...
      <CardContent className="p-4 space-y-2">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="grid gap-4">
            <p className="text-muted-foreground text-sm">Click the button below to reset your password.</p>
            {error && <Alert variant="destructive">{error}</Alert>}
            {message && <Alert variant="success">{message}</Alert>}
            <Button variant="destructive" type="submit" size="sm" className="w-fit" disabled={isLoading}>
